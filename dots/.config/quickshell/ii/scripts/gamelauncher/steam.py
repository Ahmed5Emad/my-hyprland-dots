    ...
    if not icon_path.exists():
        try:
            ...
            if response.status_code == 200:
                ...
                sys.stderr.write(f"Fetched icon for AppID {appid} and saved to {icon_path}\n")
            else:
                sys.stderr.write(f"Failed to fetch icon for AppID {appid}: HTTP {response.status_code}\n")
        except Exception as e:
            sys.stderr.write(f"Error fetching icon for AppID {appid}: {e}\n")
...
def list_games(steamapps_dirs: List[Path], fetch_icons: bool = False) -> List[Dict]:
    ...
    for sa in steamapps_dirs:
        try:
            ...
            sys.stderr.write(f"Error processing {sa}: {e}\n")
            continue
    return games
